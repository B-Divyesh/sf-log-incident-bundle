...
function demo() {
  ...
  app.innerHTML = shell(`<div class="demo-banner" role="status">Demo — sample data, nothing is saved <span><button id="reset">Reset demo</button><a href="/#install" data-route>Start for real</a></span></div><main id="main" tabindex="-1" class="demo-main"><p class="eyebrow">CHECKOUT TIMEOUT · 22 AUG 2026 · SAMPLE</p><h1>Did the retry cause duplicate charges?</h1><p class="lede">Search the six redacted sample records.</p><div class="review-tools"><label for="search">Search records</label><input id="search" type="search" placeholder="timeout, ledger, charge" autofocus><button id="download">Download CSV</button></div><p class="review-note">Sample markers show email and bearer-token redaction. The generated review lists each redaction rule.</p><div class="table-wrap" tabindex="0" aria-label="Incident records; scroll horizontally"><table><caption>Six correlated records from payment-api.log</caption><thead><tr><th>Time</th><th>Service</th><th>Level</th><th>Record</th></tr></thead><tbody id="records"></tbody></table></div><section class="finding"><h2>What the records show</h2><p>The sample ledger record confirms one charge after the retry.</p><p>The sample payment response reports <code>duplicate_charge=false</code>.</p></section></main>`);
  renderRecords();
