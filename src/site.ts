...
function shell(content: string) {
  return `<a class="skip" href="#main">Skip to content</a><header class="top"><a class="wordmark" href="/" data-route>LOG / INCIDENT<br><strong>BUNDLE</strong></a><nav aria-label="Primary"><a href="/?demo=1" data-route>Demo</a><a href="/#how" data-route>How it works</a><a href="/privacy" data-route>Privacy</a></nav></header><div class="route-status" aria-live="polite"></div>${content}<footer><p>Bounded log excerpts for incident review.</p><p><a href="/privacy" data-route>Privacy</a><a href="/terms" data-route>Terms</a><span>Built by Param Factory · v0.1.3</span></p></footer>`;
}
function landing() {
  ...
  app.innerHTML = shell(`<main id="main" tabindex="-1"><section class="hero"><div class="hero-copy"><p class="eyebrow">A LOCAL CLI FOR INCIDENT REVIEW</p><h1>Create a redacted log excerpt</h1><p class="lede">For teams who need answers without granting raw production-log access.</p><p class="hero-actions"><a class="button" href="/?demo=1" data-route>Try it with sample data</a> <span class="after-action">See a redacted incident review first.</span></p><p><a class="install-link" href="#install" data-route>Install the CLI</a> <span class="after-action">Copy a command for a machine with Rust.</span></p><ul class="facts"><li>Reads a file or standard input you choose.</li><li>Writes one self-contained HTML review copy.</li><li>MIT licensed. No account or purchase.</li></ul></div><figure><img src="/incident-press.webp" width="1200" height="800" fetchpriority="high" alt="A halftone server rack behind a clipped incident sheet." /><figcaption>A print-style view of the generated review copy.</figcaption></figure></section><section class="live-sheet" aria-labelledby="preview-heading"><div class="sheet-label">RECIPIENT VIEW · SAMPLE</div><h2 id="preview-heading">Search and export the review copy</h2>${terminalRecording()}<p><a class="sample-review-link" href="/?demo=1" data-route>Open the working sample review →</a></p></section><section id="how" class="process" aria-labelledby="how-heading"><p class="eyebrow">THREE STEPS</p><h2 id="how-heading">Make an incident review copy</h2><ol><li><strong>Choose the time window.</strong><span>Read a file or standard input.</span></li><li><strong>Follow a request or trace.</strong><span>Pull matching records into the excerpt.</span></li><li><strong>Check redactions and share.</strong><span>Send one searchable HTML file.</span></li></ol></section><section id="install" class="install" aria-labelledby="install-heading"><p class="eyebrow">INSTALL</p><h2 id="install-heading">Install the CLI</h2><p>Run this on a machine with Rust. It installs the current source from GitHub.</p><div class="install-command"><code>cargo install --git https://github.com/B-Divyesh/sf-log-incident-bundle.git --locked log-incident-bundle</code><button id="copy-install" type="button">Copy install command</button></div><p class="install-result" id="install-result" aria-live="polite"></p><p><a href="https://github.com/B-Divyesh/sf-log-incident-bundle" rel="external">Read the source on GitHub <span class="sr-only">(opens an external website)</span></a></p></section><section class="limits" aria-labelledby="limits-heading"><h2 id="limits-heading">Use it for a finite review</h2><p>The CLI creates a review copy. It is not a live log service.</p><p>Redaction is pattern-based. Review the final file before sharing it.</p></section></main>`);
  document.querySelector('#copy-install')?.addEventListener('click', copyInstallCommand);
}

const installCommand = 'cargo install --git https://github.com/B-Divyesh/sf-log-incident-bundle.git --locked log-incident-bundle';
async function copyInstallCommand() {
  const result = document.querySelector<HTMLElement>('#install-result');
  try {
    await navigator.clipboard.writeText(installCommand);
    if (result) result.textContent = 'Install command copied.';
  } catch {
    if (result) result.textContent = 'Copy the install command shown above.';
  }
}
